...
parse_dw_at_producer (annocheck_data * data, Dwarf_Attribute * attr)
{
  ...
  if (strstr (string, "NOT_FOR_PRODUCTION"))
    ...
    maybe (data, TEST_PRODUCTION, SOURCE_DW_AT_PRODUCER, "built by a cross compiler - this may not be supported");
...
check_comment_section (annocheck_data * data, annocheck_section * sec)
{
  ...
  while (tool < tool_end)
	...
	maybe (data, TEST_PRODUCTION, SOURCE_COMMENT_SECTION, "built by a cross compiler - this may not be supported");
...
static void
annobin_start_scan (uint level, const char * datafile)
{
